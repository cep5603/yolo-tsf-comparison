...
class TemporalGate(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.pos_conv = nn.Conv1d(1, channels, 3, padding=1)
        self.data_conv = nn.Conv1d(channels, channels, 1)
        self.strength = nn.Parameter(torch.ones(1, channels, 1))

    def forward(self, x):
        b, _, length = x.shape
        pos = torch.linspace(0, 1, length, device=x.device, dtype=x.dtype).view(1, 1, length)
        pos = pos.expand(b, -1, -1)
        gate = torch.sigmoid(self.pos_conv(pos) + self.data_conv(x))
        return x * (1.0 + self.strength * (gate - 0.5))

class PatchTemporalInjection(nn.Module):
    def __init__(self, patch_channels, out_channels):
        super().__init__()
        self.proj = Conv1D(patch_channels, out_channels, 1)
        self.filter = nn.Sequential(
            Conv1D(out_channels, out_channels, 3),
    ...
    def forward(self, x, patches):
        ...
        return x + self.alpha * self.filter(self.proj(patches))

class TemporalPyramidInjection(nn.Module):
    def __init__(self, patch_channels, channels):
        super().__init__()
        self.p3_inject = PatchTemporalInjection(patch_channels, channels[0])
        self.p4_inject = PatchTemporalInjection(patch_channels, channels[1])
        self.p5_inject = PatchTemporalInjection(patch_channels, channels[2])
        self.p3_gate = TemporalGate(channels[0])
        self.p4_gate = TemporalGate(channels[1])
        self.p5_gate = TemporalGate(channels[2])

    def forward(self, patches, p3, p4, p5):
        p3 = self.p3_gate(self.p3_inject(p3, patches))
        p4 = self.p4_gate(self.p4_inject(p4, patches))
        p5 = self.p5_gate(self.p5_inject(p5, patches))
        return p3, p4, p5
...
class SeparateMultiScaleForecastHead(nn.Module):
    def __init__(self, channels, horizon, dropout=0.2):
        super().__init__()
    ...
    def forward(self, feats):
        ...
        # Preserve temporal structure: flatten time
        f3 = self.dropout(f3.flatten(start_dim=1))
        f4 = self.dropout(f4.flatten(start_dim=1))
        f5 = self.dropout(f5.flatten(start_dim=1))
...
class RawStatsEncoder(nn.Module):
    def __init__(self, hidden=64, eps=1e-5):
        super().__init__()
        self.eps = eps
...
class SmoothTrendHead(nn.Module):
    def __init__(self, channels, stats_dim, horizon, controls=8):
        super().__init__()
    ...
    def forward(self, p5, stats):
        ...
        return self.scale * trend

class HorizonConditionedFusion(nn.Module):
    def __init__(self, horizon, stats_dim, hidden=64):
        super().__init__()
        self.horizon = horizon
        self.net = nn.Sequential(
            nn.Linear(stats_dim + 3, hidden),
            nn.SiLU(),
            nn.Linear(hidden, hidden),
            nn.SiLU(),
            nn.Linear(hidden, 1),
    ...
    def forward(self, f3, f4, stats):
        batch = f3.shape[0]
        pos = torch.linspace(0, 1, self.horizon, device=f3.device, dtype=f3.dtype).view(1, self.horizon, 1)
        pos = pos.expand(batch, -1, -1)
        stats = stats.unsqueeze(1).expand(-1, self.horizon, -1)
        values = torch.stack([f3, f4], dim=-1)
        fused = self.net(torch.cat([values, pos, stats], dim=-1)).squeeze(-1)
        return fused
...
class YOLO11_TSF(nn.Module):
    def __init__(
        ...
        self.stats_encoder = RawStatsEncoder(hidden=self.stats_dim)
        ...
        self.trend_head = SmoothTrendHead(
            channels=256,
            stats_dim=self.stats_dim,
            horizon=horizon,
        )

        self.fusion = HorizonConditionedFusion(
            horizon=horizon,
            stats_dim=self.stats_dim,
        )

        # Direct linear path (DLinear/NLinear-style) from the normalized input
        # window to the horizon. Zero-initialized so training starts identical
        # to the pyramid-only model and the linear continuation map is learned
        # as a residual. This gives a phase-accurate, full-resolution route to
        # the output that the stride-2 conv pyramid cannot represent.
        self.direct = nn.Linear(seq_len, horizon)
        nn.init.zeros_(self.direct.weight)
        nn.init.zeros_(self.direct.bias)
        # Learned scalar gate on the deep pyramid contribution. Initialized small
        # so that (with the ridge-initialized direct path) the model starts at the
        # linear optimum and the pyramid acts as a residual corrector that only
        # grows if it actually reduces the loss.
        self.deep_scale = nn.Parameter(torch.tensor(0.2))
    ...
    def forward(self, x, return_components=None):
        ...
        out = self.direct(x.squeeze(1)) + self.deep_scale * (detail + hp5)
