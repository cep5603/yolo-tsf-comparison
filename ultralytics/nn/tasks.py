...
from ultralytics.nn.modules import UpW
...
class ForecastModel(BaseModel):
    """
    YOLO11 forecasting model that returns an H-step vector (or HxQ).
    Input should be shaped (B, C, 1, L), i.e., a 1xL 'image'.
    """

    def __init__(self, cfg="yolo11-forecast.yaml", ch=1, nc=None, verbose=True):
        super().__init__()
        self.yaml = cfg if isinstance(cfg, dict) else yaml_model_load(cfg)
        self.yaml["channels"] = ch
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)
        self.stride = torch.Tensor([1.0])
        self.names = {0: "value"}  # Placeholder not used by loss
        self.info()

    def init_criterion(self):
        from ultralytics.utils.loss import MSEForecastLoss
        return MSEForecastLoss()
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, Forecast}
        ):
